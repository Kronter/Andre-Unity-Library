...
{
    [CustomEditor(typeof(TileAssetFourBit)), CanEditMultipleObjects]
    public class TileAssetFourBitEditor : Editor
    {
        private TileAssetFourBit myTarget;
        private int tab = 0;
        ...
        {
            myTarget = (TileAssetFourBit)target;
            if (myTarget.tiles == null)
                myTarget.tiles = new Sprite[16];
                    ...
                    if (currentSprite != null)
                    {
                        if (currentSprite != previousSprite || assets == null)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    EditorGUILayout.HelpBox(currentSprite.name + " has too few sprites, they may not be placed properly consider placing them manualy", MessageType.Warning);
                                }
                                if (sprites.Length > 16 && sprites.Length <= 48)
                                {
                                    EditorGUILayout.HelpBox(currentSprite.name + " has too many sprites, they may not be placed properly consider using EightBit Asset or placing them manualy", MessageType.Warning);
                                }
                                else if (sprites.Length > 16)
                                {
                                    EditorGUILayout.HelpBox(currentSprite.name + " has too many sprites, they may not be placed properly", MessageType.Warning);
                                }
